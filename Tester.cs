...
namespace TestingSystemConsole
{
    ...
    /// <summary>
    /// Хранит всю информацию о результатах тестирования программы
    /// </summary>
    struct TestReport
    {
        /// <summary>
        /// Название пользовательской программы
        /// </summary>
        public string userProgramName;
        /// <summary>
    ...
    class Tester
    {
        ...
        {
            report.userProgramName = pathToUserExecutableFile;
                    ...
                    if (userExecutable.Start())
                    {
                        ...
                        while (!userExecutable.HasExited)
                        {
                            ...
                            if (timer.ElapsedMilliseconds > restrictionsReader.TimeLimitInMilliseconds)
                            {
                                userExecutable.Kill();
                                timer.Stop();
                                report.results[counter].IsPassed = false;
                                report.results[counter].MemoryUsage = userExecutable.WorkingSet64;
                                report.results[counter].AverageElapsedTime = restrictionsReader.TimeLimitInMilliseconds;
                                break;
                            }
                            report.results[counter].MemoryUsage = userExecutable.WorkingSet64;
                        }
                        ...
                        if (i == 0)
                        {
                            ...
                            report.results[counter].IsPassed = answersReader.IsAnswerCorrect(output); 
                        }     
                    }
                    else
                    {
        ...
        public void SaveLogToFile(TestResult result)
        {
            ...
            logStream.WriteLine(String.Format("Тест: {0} | Статус: {1} | Время исполнения: {2:#.##} мс | Объем использованной памяти {3:#.##}Мб |\n",
                                            ...
                                            result.AverageElapsedTime, result.MemoryUsage/(1024*1024)));
            logStream.Flush();
        }
        ...
        public void ShowResultsInConsole()
        {
            for (int i = 0; i < report.results.Length; i++)
            {
                ...
                Console.WriteLine(String.Format("Тест: {0,-6} | Статус: {1,-10} | Время исполнения: {2:#.##} мс | Объем использованной памяти {3:#.##} Мб |\n",
                                            report.results[i].TestName, (report.results[i].IsPassed) ? "пройден" : "не пройден",
                                            report.results[i].AverageElapsedTime,
                                            report.results[i].MemoryUsage / (1024 * 1024)));
